import { StepperProps } from '@backstage/plugin-scaffolder-react/alpha';
...
// type not exported from @backstage/plugin-scaffolder-react/alpha, requiring manual declaration
type Steps = StepperProps['manifest']['steps'];

const WorkflowExecutionSchemaFormatter: DataFormatter<JSONSchema7, Steps> = {
  format: (data: JSONSchema7): Steps => {
    if (!data.$defs) {
      return [];
    }
    return Object.entries(data.$defs).map(([key, schema]) => {
      if (typeof schema === 'boolean') {
        throw new Error(
          `Failed to parse schema: unexpected boolean schema for key ${key}`,
        );
      }
      return {
        title: key,
...
export default WorkflowExecutionSchemaFormatter;
